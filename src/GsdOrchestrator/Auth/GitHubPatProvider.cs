    ...
    public GitHubPatProvider(IConfiguration config)
    {
        ...
        if (string.IsNullOrWhiteSpace(token))
            throw new InvalidOperationException(
                "GITHUB_TOKEN is not set. Copy .env.example to .env and add your GitHub PAT.");
        ...
        if (!token.StartsWith("ghp_", StringComparison.Ordinal) &&
            ...
            throw new InvalidOperationException(
                "GITHUB_TOKEN does not look like a valid GitHub token (expected ghp_, ghs_, or github_pat_ prefix).");
